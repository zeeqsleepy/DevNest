...
The renderer is generic: it is driven by the same JSON the machine format emits rather than by a
view written per command, which is what stops a report and a terminal run from ever describing the
same result differently. That works because the JSON conventions above are strict enough to render
from. A field ending in `Bytes` is a size and a field ending in `Ms` is a duration, so both can be
formatted; a list of flat objects is a table; a list of objects with structure of their own becomes
one section each, because `6 field(s)` in a table cell is a report that lost what it was written to
carry; and a camelCase name is a label with the spaces removed.
...
The destination directory is created if missing. An existing target file is overwritten and the
warning saying so is carried in the result, where it appears in every output format rather than
only on the terminal. There is no `--force` here, as the plan had: `clean` and `port free` already
define that flag with a meaning of their own, and one word cannot mean "delete a directory tree
without asking" in one place and "replace a report" in another.

Failure to write is a fatal error, since the user asked for a file and silently not producing one
would be worse than stopping. The file is written after the terminal output, so a person watching
sees the result even when the write fails.

The format follows the extension: `.json`, `.csv`, `.md`, `.markdown`, and `.txt`. Anything else
is a usage error naming `--export-format`, because guessing produces a file whose contents do not
match its name.
...
A command with a space in it is one argument, so `devnest export "secret scan" scan` runs two
commands. Each runs with its defaults: a flag written here could not be told apart from the next
command's name, and anything that needs flags is a single run with `--export`. Naming a group
rather than a command is a usage error that suggests one of its subcommands, and `export` refuses
to export itself.

Each section holds what that command's own envelope held, because the section is that envelope:
the commands run against a renderer that keeps the result instead of printing it, rather than
against a second code path that would drift.

## Testing

- Every format is rendered from a fixed `Result` fixture and asserted against in the test, which
  keeps a formatting change visible in the diff. No golden files: the fixtures are small enough
  that the expected output reads better beside the assertion than in a separate file nobody opens.
- CSV output is round-tripped through a parser to confirm quoting is correct.
- The exported file is read back and parsed end to end, so the path resolution, the format
  selection, and the write are covered as one thing rather than three.
- The atomic write is covered by asserting that the destination holds exactly one file afterwards:
  a temporary left behind is the failure that would otherwise go unnoticed for months.
