...
That promise is checked rather than remembered. `testdata/json-surface.txt` records every field of
every covered command as a `path: type` line, generated by `tests/json_surface_test.go` from real
runs against a fixed fixture. A rename or a removal shows up as a diff in that file, in the same
pull request that caused it, which is the only moment at which it is cheap to reconsider. Values
are not recorded: a duration and a generated password differ on every run, and neither is the
contract.
